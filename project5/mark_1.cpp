...
storing_vectors velocityVerlet(double initialVx, double initialVy, double Posx, double Posy, int n){
    ...
    for(int i = 0; i < n; i++){
        ...
        // std::cout << "-----------------------------------------------" << '\n';
        double ax = accel(sqrt(Px[i]*Px[i]+Py[i]*Py[i]), (double)Px[i]/P_init);
        double ay = accel(sqrt(Px[i]*Px[i]+Py[i]*Py[i]), (double)Py[i]/P_init);
...
int main(){
  ...
  if (not run_testfuncs){
    ...
    return 0;}
  int n = 1e+7;
  storing_vectors eul_vel_pos = eulerForward(1,1,1,0,n); //6.284
  write_to_file_vector(eul_vel_pos.Vx,eul_vel_pos.Vy,
                       eul_vel_pos.Px,eul_vel_pos.Py,
                       n, "euler_planet");
  storing_vectors vec_vel_pos = velocityVerlet(1,1,1,0,n); //6.284
